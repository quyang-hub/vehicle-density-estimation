...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            ...
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, linear=False)
...
class GroupBlock(TimmBlock):
    def __init__(self,dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, ws=1,linear=False):
...
class PatchEmbed(nn.Module):
    ...
    # pvt2 11.9
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, stride=4):
        super().__init__()
...
class PyramidVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], block_cls=Block, num_stages=4, linear=False):
        super().__init__()
        ...
        for i in range(len(depths)):
            ...
            if i == 0:
                ...
                self.patch_embeds.append(
                    PatchEmbed(img_size // patch_size // 2 ** (i + 1), 3, embed_dims[i - 1], embed_dims[i], 2))
...
def alt_gvt_large(**kwargs):
    model = ALTGVT(
        patch_size=7, embed_dims=[128, 256, 512, 1024], num_heads=[4, 8, 16, 32], mlp_ratios=[4, 4, 4, 4],
        qkv_bias=True,
